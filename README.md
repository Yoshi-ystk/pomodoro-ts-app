...
- ダークモードとライトモードの切り替え
- 設定画面からテーマを切り替え可能
- テーマ設定はアプリ再起動後も保持（AsyncStorage）
- ダークモード：ネオン風のシアングローを特徴とするダークテーマ
- ライトモード：モダンなインディゴ系のライトテーマ
...
| `src/components/`        | 再利用可能なUI部品や汎用コンポーネントを格納（TimerDisplay, CustomButton など） |
| `src/theme/`             | テーマ管理（カラーパレット、テーマコンテキスト）を格納                          |
